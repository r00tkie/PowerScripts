...
.SYNOPSIS
    ...
    code asynchronously on target Windows computers, using PsExec.exe.

    Recent versions of PsExec.exe use encrypted credentials when connecting to remote computers.
...
function Invoke-PsExec {
    ...
    begin
    {
        ...
        elseif ($PsExecExecutable = Get-Item -LiteralPath (Join-Path (Split-Path -Path $MyInvocation.MyCommand.Definition -Parent) 'PsExec.exe') -ErrorAction SilentlyContinue | Select-Object -ErrorAction SilentlyContinue -ExpandProperty FullName)
        {
        ...
        {
            Write-Error -Message "You need PsExec.exe from Microsoft's SysInternals suite to use this script. Either in the working dir, or somewhere in `$Env:PATH."
            return
        }
